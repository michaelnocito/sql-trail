  ...
  const STOPS = [
    {
      ...
      concept: 'CREATE TABLE / INSERT',
      intro: 'Before the wagon rolls, an analyst builds the tables. Guided pace: the trail gets harder from here.',
      questions: [
        {
          id: '1-1', type: 'write',
          prompt: "Create a table named provisions with columns: item (TEXT), lbs (INTEGER). No other constraints needed.",
          answer: 'CREATE TABLE provisions (item TEXT, lbs INTEGER)',
          check: "SELECT name FROM sqlite_master WHERE type='table' AND name='provisions'",
          hints: ['CREATE TABLE defines a new, empty table and its columns.',
                  'CREATE TABLE table_name (col1 TYPE, col2 TYPE)',
                  "CREATE TABLE provisions (item TEXT, lbs ___)"],
        },
        {
          id: '1-2', type: 'write',
          prompt: "The general store sold you 100 lbs of flour. Insert a row into supplies: item 'salt pork', category 'food', qty 100, unit_cost 0.45.",
          answer: "INSERT INTO supplies VALUES ('salt pork','food',100,0.45)",
          check: "SELECT item, category, qty, unit_cost FROM supplies WHERE item='salt pork'",
        ...
        },
        {
          ...
          check: 'SELECT member_id, name, role, health, morale FROM party WHERE member_id=5',
        ...
        {
          id: '1-4', type: 'write',
          prompt: "Create a table named trail_log with columns: day (INTEGER), miles (INTEGER), note (TEXT).",
          answer: 'CREATE TABLE trail_log (day INTEGER, miles INTEGER, note TEXT)',
          check: "SELECT sql FROM sqlite_master WHERE type='table' AND name='trail_log'",
          hints: ['A log table is just columns for when, how much, and what happened.',
                  'CREATE TABLE trail_log (day INTEGER, miles INTEGER, note TEXT) — match names exactly.',
                  'CREATE TABLE trail_log (day INTEGER, miles ___, note ___)'],
        },
        {
          id: '1-5', type: 'write',
          prompt: "Using explicit column names, insert into supplies only item and category and qty (unit_cost can be NULL): item 'rope', category 'parts', qty 2.",
          answer: "INSERT INTO supplies (item, category, qty) VALUES ('rope','parts',2)",
          check: "SELECT item, category, qty, unit_cost FROM supplies WHERE item='rope'",
